    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for k in range(1,max_iters):
            ...
            if z < goal_bias:
                ...
                x_rand = [np.random.uniform(0,1)*(-self.statespace_lo[0]+self.statespace_hi[0]),
                          np.random.uniform(0,1)*(-self.statespace_lo[1]+self.statespace_hi[1])]
            
            loc_near = self.find_nearest(V[0:n], x_rand)
            x_near = V[loc_near]
            ...
            if (self.is_free_motion(self.obstacles,x_new,x_near)):
                ...
                if (np.array_equal(x_new, self.x_goal)):
                    ...
                    loc_path = P[n]
                    while not (np.array_equal(V[loc_path], self.x_init)):
                        self.path.append(V[loc_path])
                        loc_path = P[loc_path]
                    self.path.append(V[loc_path])
                    self.path.reverse()
                    break
...
class GeometricRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        min_list = np.zeros(len(V))
        for i in range(0,len(V)):
            min_list[i] = np.linalg.norm(np.array(x) - np.array(V[i]))
        
        return np.argmin(min_list)
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        if (np.linalg.norm(np.array(x2) - np.array(x1)) < eps):
            ...
            return (np.array(((np.array(x2) - np.array(x1))/np.linalg.norm(np.array(x2) - np.array(x1))) * eps) + x1)
